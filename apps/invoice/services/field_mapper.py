"""Maps Azure invoice field names to CRM field names using schema configuration."""
...
from apps.invoice.exceptions import InvoiceValidationError
from apps.invoice.field_mappings import (
    CURRENCY_FIELDS,
    DATE_FIELDS,
    PARSER_ENRICHED_FIELDS,
    get_schema_mapping,
...
class MappedField:
    """A single CRM field extracted from Azure invoice output."""
...
def map_invoice_fields(
    ...
    """Map Azure invoice fields to CRM fields for the given target schema."""
    ...
    def append_mapped_field(
        ...
        *,
        ...
        if azure_field not in invoice_result.raw_fields:
            return
        if any(item.crm_field == crm_field for item in mapped.extracted):
            return
        raw_value = invoice_result.raw_fields[azure_field]
        confidence = float(invoice_result.confidence_scores.get(azure_field, 0.0))
    ...
    for azure_field in invoice_result.raw_fields:
