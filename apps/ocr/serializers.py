"""DRF serializers for OCR upload and result responses."""
...
from apps.ocr.field_mappings import list_supported_schemas
...
class OCRUploadSerializer(serializers.Serializer):
    """Validates multipart OCR extraction requests."""
    ...
    def validate_file(self, uploaded_file) -> object:
        ...
        max_bytes = int(
            getattr(settings, "OCR_MAX_UPLOAD_BYTES", MAX_UPLOAD_BYTES)
        )
...
class OCRResultSerializer(serializers.Serializer):
    """Serializes successful OCR extraction responses."""
...
class OCRErrorSerializer(serializers.Serializer):
    """Serializes OCR failure responses."""
