...
def parse_mulkiya(text,key_values=None):

    data = {"document_type":"mulkiya"}
    cleaned = re.sub(r"[ \t]+", " ", text)
    cleaned = re.sub(r"\n+", "\n", cleaned).strip()
    lines = [l.strip() for l in text.split("\n") if l.strip()]
    print("EMIRATE TEST:", normalize_emirate("دبي"))  
    full_text = "\n".join(lines)
        ...
        for i, line in enumerate(lines):
            if "traffic plate" in line.lower():
                ...
                if plate_number and plate_code:
                    ...
                    data["registration_no"] = f"{plate_code}/{plate_number}"
    ...
    if "tcf_no" not in data:
        ...
        if not tc_match:  # fallback: pick number near "T C No"
            for i, line in enumerate(lines):
                if "t" in line.lower() and "c" in line.lower():
...
# =========================
    ...
    strong_front_fields = ["registration_no", "plate_code", "plate_number", "registration_date","tcf_no","owner"]
