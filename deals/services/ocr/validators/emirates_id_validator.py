...
def validate_emirates_id(parsed_data):
    ...
    if doc_type == "emirates_id_front":
        ...
        if eid and not re.match(r"^784-\d{4}-\d{7}-\d$", eid):
        ...
        if nationality and len(nationality.strip()) < 3:
        ...
        if card and not re.fullmatch(r"\d{6,12}", str(card)):
        ...
        errors["document"] = "Unknown document type"
