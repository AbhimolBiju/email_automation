...
#         else:
#             # Reference (e.g. CreateQuote ReferenceNo / R/…) is not the same as quotation (SaveQuoteWithPlan Data / QuotNo).
#             ref = str(
#                 (
...
#                 nia_summary_section["ReferenceNo"] = ref
#             if quot:
...
class NIAProposalSummaryView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            # ProposalSummary operates on the *saved-quote* number (Q/… from SaveQuoteWithPlan).
            # Strip PolRefNo (R/… create-quote ref) from the forwarded payload to prevent
            # NIA returning "Ref No is null" when both fields are present.
            summary_payload = {k: v for k, v in payload.items() if k != "PolRefNo"}
            if "PolRefNo" in payload and not any(
                payload.get(k) for k in ("QuotNo", "quotation_no", "quote_no", "polRefNo")
            ):
                # Only keep PolRefNo when there is genuinely no QuotNo available.
                summary_payload["PolRefNo"] = payload["PolRefNo"]
            provider = _get_nia_provider()
            data = provider.proposal_summary(summary_payload)
        except LookupError as exc:
            return error_response(
                message=str(exc),
                code=status.HTTP_404_NOT_FOUND,
