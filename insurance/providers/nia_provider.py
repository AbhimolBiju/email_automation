...
class NIAProvider(BaseInsuranceProvider):
    def get_quote(self, payload: dict[str, Any]):
        ...
        endpoint = self.get_extra_config().get("quote_endpoint", "/quotes")
        response = self._request(method="POST", path=endpoint, json_payload=payload)
        return self.build_quote_from_mapping(
            {
                "premium": response.get("premium_amount"),
                "vat": response.get("vat_amount", 0),
                "total": response.get("total_amount"),
                "currency": response.get("currency", "AED"),
                "plan_name": response.get("plan_name", "NIA Plan"),
                **response,
            },
            response_time_ms=int((time.perf_counter() - started) * 1000),
        )
    ...
    def issue_policy(self, payload: dict[str, Any]) -> dict[str, Any]:
        endpoint = self.get_extra_config().get("issue_policy_endpoint", "/policies/issue")
    ...
    def renew_policy(self, payload: dict[str, Any]) -> dict[str, Any]:
        endpoint = self.get_extra_config().get("renew_policy_endpoint", "/policies/renew")
        return self._request(method="POST", path=endpoint, json_payload=payload)
